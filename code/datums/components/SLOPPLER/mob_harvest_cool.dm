/datum/component/mob_harvest_cool
	...
	/// If the used item is deleted when harvesting is complete
	var/delete_harvest_item = FALSE

//harvest_type, produced_item_typepath and speedup_type are typepaths, not reference
/datum/component/mob_harvest_cool/Initialize(harvest_tool, produced_item_typepath, produced_item_desc, max_ready, item_generation_wait, item_harvest_time, item_harvest_sound, delete_harvest_item)
	if(!isliving(parent))
	...
	src.delete_harvest_item = delete_harvest_item
	item_generation_time = item_generation_wait
	START_PROCESSING(SSobj, src)

/datum/component/mob_harvest_cool/Destroy(force)
	STOP_PROCESSING(SSobj, src)
	return ..()

/datum/component/mob_harvest_cool/process(seconds_per_tick)
...
/datum/component/mob_harvest_cool/RegisterWithParent()
	RegisterSignal(parent, COMSIG_ATOM_EXAMINE, PROC_REF(on_examine))
	RegisterSignal(parent, COMSIG_ATOM_ATTACKBY, PROC_REF(on_attackby))

///signal called on parent being examined
/datum/component/mob_harvest_cool/proc/on_examine(datum/source, mob/user, list/examine_list)
	SIGNAL_HANDLER

	if(amount_ready < 1)
		examine_list += span_notice("[parent] seems like they could use a bit more time.")
	if(amount_ready > 1)
		examine_list += span_notice("[parent] looks like they can be harvested about [amount_ready] times, use a [initial(harvest_tool.name)].")
	if(amount_ready == 1)
		examine_list += span_notice("[parent] looks ready to be harvested, use a [initial(harvest_tool.name)].")

///signal called on parent being attacked with an item
/datum/component/mob_harvest_cool/proc/on_attackby(datum/source, obj/item/used_item, mob/user)
	SIGNAL_HANDLER

	if(istype(used_item, harvest_tool))
		INVOKE_ASYNC(src, PROC_REF(harvest_item), user, used_item)
