/datum/component/grillable
	...
	/// Do we transfer reagents from the old cooked item? // DOPPLER EDIT ADDITION
	var/transfer_old_reagents = FALSE // DOPPLER EDIT ADDITION
...
/datum/component/grillable/Initialize(cook_result, required_cook_time, positive_result, use_large_steam_sprite, list/added_reagents, transfer_old_reagents)
	...
	src.transfer_old_reagents = transfer_old_reagents // DOPPLER EDIT ADDITION
...
/datum/component/grillable/proc/finish_grilling(atom/grill_source)
	...
	if(IsEdible(grilled_result) && positive_result)
		...
		if(transfer_old_reagents) // DOPPLER EDIT ADDITION START
			grilled_result.reagents.clear_reagents()
			original_object.reagents?.trans_to(grilled_result, original_object.reagents.total_volume) // DOPPLER EDIT END
	...
	if(!isstructure(grilled_result)) // DOPPLER EDIT ADDITION START
		grilled_result.pixel_x = original_object.pixel_x
		grilled_result.pixel_y = original_object.pixel_y // DOPPLER EDIT END
	qdel(parent)
