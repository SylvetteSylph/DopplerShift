...
/obj/structure/rimworld_campfire
	name = "firepit"
	desc = "A stack of wood and other burnable stuff that makes light and cooks food."
	icon = 'modular_sloppler/resources/icons/campfire.dmi'
	icon_state = "campfire"
...
/obj/structure/rimworld_campfire/Initialize(mapload)
	. = ..()
...
/obj/structure/rimworld_campfire/Destroy()
	STOP_PROCESSING(SSobj, src)
	QDEL_NULL(burning_loop)
	return ..()

/obj/structure/rimworld_campfire/examine(mob/user)
	. = ..()
	if(!burning)
		. += span_notice("You can light this by interacting with empty hands.")
	. += span_notice("It has roughly <b>[round(fuel_remaining / (1 MINUTES), 0.1)] minute(s)</b> of fuel left.")

/obj/structure/rimworld_campfire/attackby(obj/item/used_item, mob/living/user, params)
...
/obj/structure/rimworld_campfire/attack_hand(mob/user, list/modifiers)
	. = ..()
	if(.)
		return
	if(burning)
...
/obj/structure/rimworld_campfire/proc/start_burning()
	if(!fuel_remaining)
		return
	if(burning)
		return
	...
	icon_state = "fire_burning"
	START_PROCESSING(SSobj, src)

/obj/structure/rimworld_campfire/fire_act(exposed_temperature, exposed_volume)
	start_burning()

/// Checks what to do if something or someone enters the campfire's turf
/obj/structure/rimworld_campfire/proc/on_entered(datum/source, atom/movable/entered)
	SIGNAL_HANDLER
	if(burning)
		if(!grill)
			bonfire_burn()
...
/obj/structure/rimworld_campfire/proc/bonfire_burn(seconds_per_tick = 2)
...
/obj/structure/rimworld_campfire/process(seconds_per_tick)
	if(!fuel_remaining)
		extinguish()
		qdel(src)
		return
...
/obj/structure/rimworld_campfire/extinguish()
	. = ..()
	if(!burning)
		return
